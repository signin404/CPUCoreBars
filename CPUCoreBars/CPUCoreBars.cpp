...
// =================================================================
CHardwareMonitorItem::CHardwareMonitorItem(const std::wstring& identifier, const std::wstring& label_text) : m_identifier(identifier), m_label_text(label_text), m_value_text(L"--"), m_sample_text(L"100°C") { m_name = L"HWMon " + label_text; m_id = L"hwmon_" + label_text; }
...
void CHardwareMonitorItem::UpdateValue()
{
    try
    {
        ...
        if (sensor != nullptr) { m_value_text = HardwareMonitor::StringToStdWstring(HardwareMonitor::GetSensorValueText(sensor)); }
        else { m_value_text = L"N/A"; }
    }
...
CCPUCoreBarsPlugin::CCPUCoreBarsPlugin() : m_cached_whea_count(0), m_cached_nvlddmkm_count(0), m_last_error_check_time(0)
{
...
CCPUCoreBarsPlugin::~CCPUCoreBarsPlugin() { if (m_query) PdhCloseQuery(m_query); for (auto item : m_plugin_items) delete item; ShutdownNVML(); ShutdownHardwareMonitor(); GdiplusShutdown(m_gdiplusToken); }
IPluginItem* CCPUCoreBarsPlugin::GetItem(int index) { if (index >= 0 && index < static_cast<int>(m_plugin_items.size())) { return m_plugin_items[index]; } return nullptr; }
...
void CCPUCoreBarsPlugin::InitHardwareMonitor()
{
    try
    {
        ...
        if (cpu_temp_sensor != nullptr) { std::wstring identifier = HardwareMonitor::StringToStdWstring(HardwareMonitor::GetSensorIdentifier(cpu_temp_sensor)); m_cpu_temp_item = new CHardwareMonitorItem(identifier, L"CPU Max"); m_plugin_items.push_back(m_cpu_temp_item); }
        if (gpu_temp_sensor != nullptr) { std::wstring identifier = HardwareMonitor::StringToStdWstring(HardwareMonitor::GetSensorIdentifier(gpu_temp_sensor)); m_gpu_temp_item = new CHardwareMonitorItem(identifier, L"GPU"); m_plugin_items.push_back(m_gpu_temp_item); }
    }
    catch (Exception^) { /* Handle initialization error */ }
}
void CCPUCoreBarsPlugin::ShutdownHardwareMonitor() { if (m_computer) { m_computer->Close(); m_computer = nullptr; } }
void CCPUCoreBarsPlugin::UpdateHardwareMonitorItems() { if (m_computer) { m_computer->Accept(m_updateVisitor); if (m_cpu_temp_item) m_cpu_temp_item->UpdateValue(); if (m_gpu_temp_item) m_gpu_temp_item->UpdateValue(); } }
...
DWORD CCPUCoreBarsPlugin::QueryEventLogCount(LPCWSTR provider_name) { wchar_t query[256]; swprintf_s(query, L"*[System[Provider[@Name='%s'] and TimeCreated[timediff(@SystemTime) <= 86400000]]]", provider_name); EVT_HANDLE hResults = EvtQuery(NULL, L"System", query, EvtQueryChannelPath | EvtQueryReverseDirection); if (hResults == NULL) return 0; DWORD total_count = 0; EVT_HANDLE hEvents[256]; DWORD returned = 0; while (EvtNext(hResults, ARRAYSIZE(hEvents), hEvents, 1000, 0, &returned)) { total_count += returned; for (DWORD i = 0; i < returned; i++) EvtClose(hEvents[i]); } EvtClose(hResults); return total_count; }
void CCPUCoreBarsPlugin::UpdateWheaErrorCount() { m_cached_whea_count = QueryEventLogCount(L"WHEA-Logger"); m_whea_error_count = m_cached_whea_count; }
void CCPUCoreBarsPlugin::UpdateNvlddmkmErrorCount() { m_cached_nvlddmkm_count = QueryEventLogCount(L"nvlddmkm"); m_nvlddmkm_error_count = m_cached_nvlddmkm_count; }
extern "C" __declspec(dllexport) ITMPlugin* TMPluginGetInstance() { return &CCPUCoreBarsPlugin::Instance(); }

// ADD THIS LINE AT THE VERY END OF THE FILE
// This forces the compiler to include the implementation of UpdateVisitor.
#include "UpdateVisitor.cpp"