...
void CCpuUsageItem::SetIsECore(bool is_e_core) { m_is_e_core = is_e_core; } // 新增函数的实现
    ...
    : m_is_running(true), m_is_initialized(false)
{
    ...
    // 2. 立即创建所有监控项，默认它们都不是E-Core
    for (int i = 0; i < m_num_cores; ++i)
    {
        m_items.push_back(new CCpuUsageItem(i, false));
    }
...
void CCPUCoreBarsPlugin::WorkerThread()
{
    ...
    for (int i = 0; i < m_num_cores; ++i)
    {
        ...
        m_items[i]->SetIsECore(is_e_core);
    }
...
const wchar_t* CCPUCoreBarsPlugin::GetInfo(PluginInfoIndex index)
{
    ...
    case TMI_VERSION: return L"1.8.0"; // 最终稳定版
    default: return L"";
    }
}
