...
// =================================================================
// UPDATED: CNvidiaLimitReasonItem implementation (font size unified)
// =================================================================
...
// UPDATED: DrawItem now uses the default font for everything
void CNvidiaLimitReasonItem::DrawItem(void* hDC, int x, int y, int w, int h, bool dark_mode)
{
    HDC dc = (HDC)hDC;

    // 1. Prepare icon text and colors
    ...
    // 2. Measure the current icon text to define layout dynamically
    ...
    // 3. Draw Icon (using the default font)
    if (is_whea_error) {
        SetTextColor(dc, RGB(255, 0, 0)); // Red for errors
    } else {
        SetTextColor(dc, default_text_color);
    }
    DrawTextW(dc, icon_text, -1, &icon_rect, DT_LEFT | DT_VCENTER | DT_SINGLELINE);

    // 4. Draw Value Text (using the same default font)
    SetTextColor(dc, default_text_color);
...
void CCPUCoreBarsPlugin::DataRequired() { UpdateCpuUsage(); UpdateGpuLimitReason(); UpdateWheaErrorCount(); if (m_gpu_item) { m_gpu_item->SetWheaCount(m_whea_error_count); } }
const wchar_t* CCPUCoreBarsPlugin::GetInfo(PluginInfoIndex index) { switch (index) { case TMI_NAME: return L"CPU/GPU 性能监视器"; case TMI_DESCRIPTION: return L"显示CPU核心使用率和NVIDIA GPU/WHEA状态。"; case TMI_AUTHOR: return L"Your Name"; case TMI_COPYRIGHT: return L"Copyright (C) 2025"; case TMI_URL: return L""; case TMI_VERSION: return L"2.6.0"; default: return L""; } }
