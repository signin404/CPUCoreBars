...
#include "resource.h"       // 包含资源ID
#include <string>
#include <fstream>
#include <vector>
#include <CommDlg.h>        // 包含通用对话框 (颜色选择器)
#include <Pdh.h>
#include <PdhMsg.h>
...
CCpuUsageItem::CCpuUsageItem(int core_index, bool is_e_core)
    : m_core_index(core_index), m_is_e_core(is_e_core), m_color(RGB(0, 128, 0)) // 默认颜色
{
...
void CCpuUsageItem::SetColor(COLORREF color)
{
    m_color = color;
}
...
INT_PTR CALLBACK SettingsDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DRAWITEM:
    {
        if (wParam == IDC_COLOR_PREVIEW)
        {
            ...
            int selected_index = SendMessage(GetDlgItem(hDlg, IDC_CORE_LIST), LB_GETCURSEL, 0, 0);
...
{
    m_core_colors.assign(m_num_cores, RGB(0, 128, 0)); // Default color
...
const wchar_t* CCPUCoreBarsPlugin::GetInfo(PluginInfoIndex index)
{
    ...
    case TMI_VERSION: return L"2.0.0"; // Final version
    default: return L"";
    }
}
