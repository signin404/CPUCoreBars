...
// =================================================================
// NEW: Merged CNvidiaStatusItem implementation
// =================================================================
CNvidiaStatusItem::CNvidiaStatusItem()
{
    wcscpy_s(m_label_text, L"GPU P-State:");
    wcscpy_s(m_value_text, L"N/A");
}
const wchar_t* CNvidiaStatusItem::GetItemName() const { return L"NVIDIA 性能状态"; }
const wchar_t* CNvidiaStatusItem::GetItemId() const { return L"nvidia_status"; }
const wchar_t* CNvidiaStatusItem::GetItemLableText() const { return m_label_text; }
const wchar_t* CNvidiaStatusItem::GetItemValueText() const { return m_value_text; }
const wchar_t* CNvidiaStatusItem::GetItemValueSampleText() const { return L"硬过热"; }
bool CNvidiaStatusItem::IsCustomDraw() const { return false; }
int CNvidiaStatusItem::GetItemWidth() const { return 0; }
void CNvidiaStatusItem::DrawItem(void* hDC, int x, int y, int w, int h, bool dark_mode) { }
void CNvidiaStatusItem::SetValue(nvmlPstate_t pstate, const wchar_t* reason_text)
{
    if (pstate <= NVML_PSTATE_15) {
        swprintf_s(m_label_text, L"P%d:", pstate);
    } else {
        wcscpy_s(m_label_text, L"GPU:");
    }
...
CCPUCoreBarsPlugin::~CCPUCoreBarsPlugin()
{
    ...
    if (m_gpu_status_item) delete m_gpu_status_item;
    ShutdownNVML();
}
...
IPluginItem* CCPUCoreBarsPlugin::GetItem(int index)
{
    ...
    if (index == m_num_cores && m_gpu_status_item) return m_gpu_status_item;
    return nullptr;
}
...
const wchar_t* CCPUCoreBarsPlugin::GetInfo(PluginInfoIndex index)
{
    ...
    case TMI_DESCRIPTION: return L"显示CPU核心使用率和NVIDIA GPU性能状态。";
    case TMI_AUTHOR: return L"Your Name";
    case TMI_COPYRIGHT: return L"Copyright (C) 2025";
    case TMI_URL: return L"";
    case TMI_VERSION: return L"2.3.0";
    default: return L"";
    }
}
...
void CCPUCoreBarsPlugin::InitNVML()
{
    ...
    m_gpu_status_item = new CNvidiaStatusItem();
}
...
{
    if (!m_nvml_initialized || !m_gpu_status_item) return;
    ...
    pfn_nvmlDeviceGetPerformanceState(m_nvml_device, &pstate);
    
    m_gpu_status_item->SetValue(pstate, reason_text);
}
