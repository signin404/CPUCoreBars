...
const wchar_t* CNvidiaMonitorItem::GetItemLableText() const { return L""; } // No longer used
...
void CNvidiaMonitorItem::DrawItem(void* hDC, int x, int y, int w, int h, bool dark_mode)
{
    ...
    // 1. Define icon area and colors
    int icon_size = min(w, h) - 2; // Make icon slightly smaller than the full height
    ...
    Ellipse(dc, icon_rect.left, icon_rect.top, icon_rect.right, icon_rect.bottom);
    SelectObject(dc, hOldBrush);
    SelectObject(dc, hOldPen);
    DeleteObject(hBrush);
    DeleteObject(hPen);

    // 3. Draw the P-State number inside the circle
    ...
    // 4. Draw the value text (limit reason)
...
void CCPUCoreBarsPlugin::InitNVML()
{
    ...
    if (!pfn_nvmlInit || !pfn_nvmlShutdown || !pfn_nvmlDeviceGetHandleByIndex || !pfn_nvmlDeviceGetCurrentClocksThrottleReasons || !pfn_nvmlDeviceGetPerformanceState) {
        ShutdownNVML();
        return;
    }
...
void CCPUCoreBarsPlugin::UpdateGpuPState()
{
    if (!m_nvml_initialized) return;
    pfn_nvmlDeviceGetPerformanceState(m_nvml_device, &m_p_state);
}
